...
def obtener_cajas_instrumental():
    ...
    try:
        cursor = conn.cursor()
        cursor.execute('SELECT * FROM "EC_SBO_BIOCELLS_PROD"."BIOCELLS_CAJASINSTRUMENTAL"')
...
def warehouses():
    try:
        ...
        FROM "EC_SBO_BIOCELLS_PROD"."BIOCELLS_Bodegas"
        """
...
def carga_masiva_cajas():
    ...
    try:
        ...
        for _, row in df_encabezado.iterrows():
            ...
            cursor.execute("""SELECT COUNT(*) FROM "EC_SBO_BIOCELLS_PROD"."@LS_CAJ_CAB" WHERE "Code" = ?""", (codigo_caja,))
            if cursor.fetchone()[0] > 0:
